...
Main.prototype.getOrCreatePortal = function(matrix_user, peer_type, peer_id) {
    return this.getPortal(matrix_user, peer_type, peer_id, true);
};

Main.prototype.getPortal = function(matrix_user, peer_type, peer_id, create) {
    var matrix_user_id = matrix_user.userId();
    var key = [matrix_user_id, peer_type, peer_id].join(" ");
    ...
    return this._bridge.getRoomStore().select({
        ...
        // Create it
        var peer = new TelegramGhost.Peer(peer_type, peer_id);
...
Main.prototype.onTelegramUpdate = function(matrix_user, peer, update) {
    // TODO peer gutwrench
    return this.getPortal(matrix_user, peer._type, peer._id).then((portal) => {
        if (!portal) return;
