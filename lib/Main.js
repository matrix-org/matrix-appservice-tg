...
Main.prototype.getTelegramUserFor = function(opts) {
    var user_id;
    if     ("user_id" in opts) user_id = opts.user_id;
    else if("user" in opts)    user_id = opts.user.id;
    else
        throw new Error("TODO: require 'user_id' or 'user' opt");

    var intent = this._bridge.getIntentFromLocalpart(
        this.username_template.expandLocalpart({ID: user_id})
    );

    // TODO: cache them
    return new TelegramUser(this, {
        intent: intent,
    });
}
