...
function TelegramGhost(opts) {
    ...
    // TODO this._dataCenter = opts.data_center || TelegramLink.PROD_PRIMARY_DC;
...
TelegramGhost.prototype.sendMessage = function(peer, text) {
    ...
    return this._doWithRedirect((client) => {
        ...
        return client.messages.sendMessage(
            peer.toInputPeer(),
            text,
            Math.random() * (1<<30), null, null, null
        );
    }).then((result) => {
...
Peer.prototype.toInputPeer = function() {
    switch(this._type) {
        case "chat":
            return new TelegramLink.type.InputPeerChat({props: {
                chat_id: this._id,
            }});

        case "channel":
            return new TelegramLink.type.InputPeerChannel({props: {
                channel_id:  this._id,
                access_hash: this._access_hash,
            }});
