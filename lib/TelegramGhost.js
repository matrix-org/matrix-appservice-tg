...
TelegramGhost.prototype.sendCode = function(phone_number) {
    return this._getClient().then((client) => {
        // It's possible that we get a redirection error sending us to a different
        //   data centre.
        return _sendCode(client, phone_number).catch((result) => {
            if (result.error_code != 303) throw result;

            // The error message will contain the index number of the DC we're
            // asked to migrate to. Yes... this makes me sad.
            var index = result.error_message.match(/(\d+)$/)[1];
