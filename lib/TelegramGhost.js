...
function TelegramGhost(opts) {
    ...
    if (opts.auth_key_buffer && opts.auth_key_password) {
        ...
        // TODO(paul): this will fail with MSG_WAIT_FAILED if the user is
        //   part-way through 2FA
        this.start();
    }
}
...
TelegramGhost.prototype.getChatInfo = function(peer) {
    if (peer._type === "chat") throw new Error("Cannot get chat info on users");

    return this._getClient().then((client) => {
        if (peer._type === "chat"   ) return client.messages.getFullChat(peer._id);
        if (peer._type === "channel") return client.channels.getFullChannel(peer.toInputChannel());
        throw new Error("Impossible");
    }).then((ret) => {
        // Assemble all the useful information about the chat
        return {
            title: ret.chats.list[0].title,
        };
    });
};

TelegramGhost.prototype._onTelegramUpdate = function(updates) {
    // The structure here has many possibilities
    var type = updates.getTypeName().replace(/^api\.type\./, "");

    switch(type) {
        case "UpdateShort":
            this._onOneUpdate(updates.update);
            break;

        case "Updates":
            updates.updates.list.forEach((update) => {
                this._onOneUpdate(update);
            });
            break;

        case "UpdateShortChatMessage":
            this._onOneUpdate(updates);
            break;
...
TelegramGhost.prototype._onOneUpdate = function(update) {
    ...
    switch(type) {
        ...
        case "UpdateShortChatMessage":
        {
            ...
            this._main.onTelegramUpdate(this._user, peer, update);
            break;
        }
