...
Portal.prototype._fixParticipants = function(participants) {
    ...
    return Promise.all(
        participants.map((p) => {
            ...
            var intent = main.getMatrixGhostFor(p);
...
Portal.prototype.onTelegramUpdate = function(update, hints) {
    ...
    switch(type) {
        ...
        case "UpdateShortChatMessage":
            var user = users.getById(update.from_id);
            console.log(` | user ${update.from_id} sent message`);
            user_intent = this._main.getMatrixGhostFor(user);
